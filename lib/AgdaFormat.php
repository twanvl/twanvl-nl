...
class AgdaFormat extends HaskellFormat {
  ...
  public function format($code) {
    if (preg_match('@--\s*HIDE (.*)|{-\s*HIDE (.*)\s*-}@', $code, $ma)) {
      $to_hide = $ma[1];
      $code = preg_replace('@\s*--\s*HIDE (.*)|{-\s*HIDE (.*)\s*-}@', '', $code);
      $code = preg_replace("@[{](($to_hide)\s*)+[}]\s*|\\s+\\b($to_hide)\\b|\\b($to_hide)\\b\\s*\\s*@u", '', $code);
    }
...
$agda_lang['keyword'] = '@\b(if|then|else|module|open|import|qualified|hiding|where|let|in|case|of|newtype|default|infix|infixr|infixl|data|class|instance|forall|exists|deriving|do|record|field|open|public|private|renaming|using|as|postulate|with|mutual|syntax|__keyword__[[:alnum:]]+)(?=$|[ (){}])@u';
